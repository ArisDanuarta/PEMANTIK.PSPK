...
export function parseFlexibleDate(dateInput: any): string | null {
  ...
  if (!result) {
    ...
    if (textMatch) {
      ...
      if (monthMap[monthStr]) {
        result = `${year}-${monthMap[monthStr]}-${day}`;
      }
    }
  }
  ...
  if (!result) {
    ...
    if (!isNaN(fallbackDate.getTime())) {
      result = fallbackDate.toISOString().split("T")[0];
    }
  }
  ...
  if (result) {
    ...
    // Fix typos like "0217" -> 2017
    if (year > 0 && year < 100) {
      year += 2000;
    } else if (year >= 100 && year < 1000) {
      year += 1800; // e.g. 217 -> 2017
    }

    const currentYear = new Date().getFullYear();
    // Validate range (students shouldn't be born before 1990 or in the future)
    if (year < 1990 || year > currentYear) {
      return null;
    }
    
    parts[0] = String(year).padStart(4, '0');
    return parts.join('-');
  }
