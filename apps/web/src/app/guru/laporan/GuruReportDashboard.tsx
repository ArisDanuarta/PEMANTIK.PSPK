...
export default function GuruReportDashboard({ packages, classes, schoolId, teacherId }: Props) {
  ...
  const handlePackageChange = useCallback(async (pkgId: string) => {
    ...
    try {
      const res = await fetch(`/api/guru/report?category_id=${pkgId}`);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      a.download = `Laporan_Guru_${selectedPackageId.substring(0, 8)}.xlsx`;
