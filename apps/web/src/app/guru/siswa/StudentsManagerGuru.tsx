...
export default function StudentsManagerGuru({ initialStudents, classes, schoolId, sesVariables }: Props) {
  ...
  const handleDownloadTemplate = () => {
    ...
    XLSX.writeFile(wb, "Template_Siswa_Guru.xlsx");
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      <div className="card" style={{ padding: 0, overflow: "hidden" }}>
        <table className="pemantik-table" style={{ width: "100%" }}>
          <thead>
            <tr><th>Nama Siswa</th><th>NISN &amp; Username</th><th>Kelas</th><th>Gender</th><th>SES</th><th>Status</th><th>Aksi</th></tr>
          </thead>
          <tbody>
            {filtered.length === 0 ? (
              ...
              <tr key={s.id}>
                ...
                <td>
                  <div style={{ fontSize: "0.85rem" }}>{s.nisn ?? "—"}</div>
                  {s.username && <div style={{ fontSize: "0.78rem", color: "#6c757d" }}>@{s.username}</div>}
                </td>
                <td>
                  {s.classes ? (
      ...
      {isBulkModalOpen && mounted && (
        <BulkUploadModal
          ...
          templateFileName="template_siswa_guru.xlsx"
