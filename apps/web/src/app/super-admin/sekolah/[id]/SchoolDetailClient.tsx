...
interface School {
  ...
  is_active: boolean;
  communities: { id: string; name: string; code: string } | null;
  users?: any[];
}
...
export default function SchoolDetailClient({ school, teachers, students, classes, importBatches, communities, schoolAdmin, sesVariables, stages, sessions }: Props) {
  const { success: showSuccess, error: showError } = useToast();
  const { confirm } = useConfirm();
  const [activeTab, setActiveTab] = useState<"ringkasan" | "guru" | "siswa" | "kelas" | "ekspor">("ringkasan");
  ...
  const filteredStudents = students.filter((s) => {
    const matchKelas = filterKelas === "all" || s.class_id === filterKelas;
    const matchSearch =
  ...
  const handlePollStatus = async (batchId: string) => {
  ...
  const handleDownloadGuruTemplate = () => {
    ...
    const wsData = [
      headers,
      ["Budi Santoso", "198001012005011003", "budi@contoh.com", "L", "1980-01-01", school.name, classes[0]?.name || "5A", school.village || "Menteng", school.district || "Menteng", school.city || "Jakarta Pusat", school.province || "DKI Jakarta"]
    ];
    ...
    const petunjukData = [
      ...
      ["email_guru", "Tidak", "Email aktif guru (opsional)."],
      ["jenis_kelamin", "Ya", "L untuk Laki-laki, P untuk Perempuan."],
      ["tanggal_lahir", "Ya", "Format YYYY-MM-DD. Contoh: 1985-06-15"],
      ["nama_sekolah", "Ya", `Harus sama persis dengan: ${school.name}`],
      ["kelas", "Ya", "Kelas yang diajar, pisahkan koma jika lebih dari satu. Contoh: 5A, 5B"],
    ...
    XLSX.writeFile(wb, `Template_Guru_${school.name.replace(/[^a-zA-Z0-9]/g, "_")}.xlsx`);
  };
  ...
  const handleResetTeacherPass = async (row: any) => {
    const isConfirmed = await confirm({ title: "Reset Sandi", description: `Reset sandi ${row.full_name} ke default?`, confirmLabel: "Reset", cancelLabel: "Batal" });
    if (!isConfirmed) return;
    startTransition(async () => {
      const res = await resetTeacherPasswordAction(row.id);
      if (res.success) showSuccess("Berhasil", "Sandi direset.");
      else showError("Gagal", res.error || "Gagal reset sandi.");
    });
  };
  ...
  const handleResetStudentPass = async (row: any) => {
    const isConfirmed = await confirm({ title: "Reset Sandi", description: `Reset sandi ${row.full_name}?`, confirmLabel: "Reset", cancelLabel: "Batal" });
    if (!isConfirmed) return;
    startTransition(async () => {
      const res = await resetStudentPasswordAction(row.id);
      if (res.success) showSuccess("Berhasil", "Sandi direset.");
      else showError("Gagal", res.error || "Gagal reset sandi.");
    });
  };
  ...
  const handleDeleteClass = async (row: any) => {
    ...
    startTransition(async () => {
      ...
      if (res.success) showSuccess("Berhasil", "Kelas dihapus.");
      else showError("Gagal", res.error || "Gagal menghapus.");
    });
  };
  ...
  const handleExportAccounts = () => {
    try {
      ...
      if (wb.SheetNames.length === 0) {
        showError("Gagal", "Tidak ada data akun untuk diekspor.");
        return;
      }
      ...
      showSuccess("Berhasil", "File data akun berhasil diunduh.");
    } catch (err) {
      console.error(err);
      showError("Gagal", "Terjadi kesalahan saat meng-export data akun.");
    }
  };
  ...
  return (
    <div className="card" style={{ marginTop: "1rem" }}>
      {/* Tab Navigation */}
      <div style={{ display: "flex", borderBottom: "2px solid #e5e7eb", padding: "0 1.5rem" }}>
        {tabs.map((tab) => (
          <button
            key={tab.id}
            ...
            style={{
              ...
              borderBottom: `2px solid ${activeTab === tab.id ? "#102e50" : "transparent"}`,
              marginBottom: -2,
      ...
      <div style={{ padding: "1.5rem" }}>
        ...
        {activeTab === "ringkasan" && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem", marginBottom: "2rem" }}>
              ...
              <div>
                <h3 style={{ fontSize: "1rem", fontWeight: 700, color: "#102e50", marginBottom: "1rem" }}>Status Import Dapodik</h3>
                {school.import_source === "dapodik" ? (
                  <div>
                    <div style={{ padding: "0.75rem", background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: "0.5rem", marginBottom: "1rem" }}>
                      <span style={{ color: "#166534", fontWeight: 600, fontSize: "0.9rem" }}>✅ Ter-import dari Dapodik</span>
                ...
                <Button
                  ...
                  style={{ backgroundColor: "#102e50", color: "white", width: "100%" }}
                >
                  {school.import_source === "dapodik" ? "Update Data Dapodik" : "Import Dapodik"}
                </Button>
              </div>
            </div>
            ...
            {importBatches.length > 0 && (
              <div>
                <h3 style={{ fontSize: "1rem", fontWeight: 700, color: "#102e50", marginBottom: "1rem" }}>Riwayat Import</h3>
                <div style={{ border: "1px solid #e5e7eb", borderRadius: "0.5rem", overflow: "hidden" }}>
                  {importBatches.map((batch, i) => (
                    <div key={batch.id} style={{ padding: "0.875rem 1rem", borderBottom: i < importBatches.length - 1 ? "1px solid #f3f4f6" : "none", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      <div>
                        <div style={{ fontWeight: 600, fontSize: "0.85rem", color: "#374151" }}>
                          {new Date(batch.created_at).toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" })}
                        </div>
                        <div style={{ fontSize: "0.8rem", color: "#6b7280" }}>
                          {batch.success_count}/{batch.total_rows} siswa berhasil
                          {batch.fail_count > 0 && ` • ${batch.fail_count} gagal`}
                        </div>
                      </div>
        ...
        {activeTab === "guru" && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.25rem", flexWrap: "wrap", gap: "0.75rem" }}>
              <h3 style={{ fontSize: "1rem", fontWeight: 700, color: "#102e50", margin: 0 }}>Daftar Guru ({teachers.length})</h3>
              <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
                <Button
                  variant="outline"
                  onClick={handleDownloadGuruTemplate}
                  style={{ color: "#0874aa", borderColor: "#0874aa", fontSize: "0.85rem" }}
                >
                ...
                <Button onClick={() => setIsTeacherModalOpen(true)} style={{ backgroundColor: "#102e50", color: "white", fontSize: "0.85rem" }}>+ Tambah Guru</Button>
              </div>
            </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.25rem", flexWrap: "wrap", gap: "0.75rem" }}>
              ...
              <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap" }}>
                <input
                  ...
                  style={{ padding: "0.4rem 0.75rem", border: "1px solid #d1d5db", borderRadius: "0.375rem", fontSize: "0.85rem", width: 200 }}
                />
                <select
                  value={filterKelas}
                  onChange={(e) => setFilterKelas(e.target.value)}
                  style={{ padding: "0.4rem 0.75rem", border: "1px solid #d1d5db", borderRadius: "0.375rem", fontSize: "0.85rem" }}
                >
        ...
        {activeTab === "kelas" && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.25rem" }}>
              <h3 style={{ fontSize: "1rem", fontWeight: 700, color: "#102e50", margin: 0 }}>Daftar Kelas</h3>
            ...
            {classes.length === 0 ? (
              <EmptyState message="Belum ada kelas yang terdaftar." hint="Kelas akan dibuat otomatis saat import Dapodik, atau tambah manual melalui tombol di atas." />
            ) : (
                  ...
                  return (
                    <div key={c.id} style={{ border: "1px solid #e5e7eb", borderRadius: "0.75rem", padding: "1.25rem", background: "white", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "0.75rem" }}>
                        <div>
                          <div style={{ fontWeight: 700, fontSize: "1rem", color: "#102e50" }}>{c.name}</div>
                          <div style={{ fontSize: "0.75rem", color: "#6b7280" }}>Kelas {c.grade} • {c.academic_year}</div>
                        </div>
        ...
        {activeTab === "ekspor" && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.25rem" }}>
              <h3 style={{ fontSize: "1rem", fontWeight: 700, color: "#102e50", margin: 0 }}>Ekspor Data Akun Pengguna</h3>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "1rem" }}>
              <div style={{ border: "1px solid #e5e7eb", borderRadius: "0.5rem", padding: "1.25rem", background: "white" }}>
                ...
                <div style={{ fontWeight: 700, fontSize: "1.1rem" }}>{schoolAdmin ? 1 : 0} Akun Sekolah</div>
        ...
        <BulkUploadModal
          ...
          onDapodikConfirm={async (payload) => {
            const enrichedPayload = {
              ...payload,
              school_choice: "existing" as const,
              existing_school_id: school.id,
            };
            const result = await importDapodikAction(enrichedPayload);
            return result;
          }}
...
function InfoRow({ label, value, mono = false }: { label: string; value: string; mono?: boolean }) {
  return (
    <div style={{ display: "flex", gap: "0.5rem", marginBottom: "0.5rem", fontSize: "0.875rem" }}>
      <span style={{ color: "#6b7280", minWidth: 130 }}>{label}:</span>
      <span style={{ color: "#111827", fontFamily: mono ? "monospace" : "inherit", fontWeight: mono ? 500 : 400 }}>{value}</span>
    </div>
  );
}
...
function EmptyState({ message, hint }: { message: string; hint?: string }) {
  return (
    <div style={{ padding: "3rem", textAlign: "center", color: "#6b7280" }}>
      ...
      <div style={{ fontWeight: 600, marginBottom: "0.375rem" }}>{message}</div>
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; color: string; bg: string }> = {
    queued: { label: "Antrian", color: "#6b7280", bg: "#f3f4f6" },
    processing: { label: "Proses...", color: "#0369a1", bg: "#e0f2fe" },
    completed: { label: "Selesai", color: "#166534", bg: "#dcfce7" },
    completed_with_errors: { label: "Selesai (ada error)", color: "#92400e", bg: "#fef3c7" },
    failed: { label: "Gagal", color: "#991b1b", bg: "#fee2e2" },
  };
  const s = map[status] ?? { label: status, color: "#6b7280", bg: "#f3f4f6" };
  return (
    <span style={{ fontSize: "0.75rem", fontWeight: 600, padding: "0.2rem 0.6rem", borderRadius: 999, color: s.color, background: s.bg }}>
      {s.label}
    </span>
  );
}
