...
// jadi 2 path datar — sebelumnya dibungkus banyak nested clipPath yang
// sebenarnya menghasilkan bentuk yang sama).
const PATH_YELLOW =
...
interface PemantikLogoProgressProps {
  /** Nilai saat ini (misal: 3) */
  value: number;
  /** Nilai maksimum / target (misal: 10 untuk 3/10) */
  max: number;
  /** Ukuran logo dalam px (persegi). Default 160 */
  size?: number;
  /** Durasi animasi fill dalam ms. Default 900 */
  durationMs?: number;
  /** Warna base sebelum terisi. Default abu-abu Tailwind gray-300 */
  emptyColor?: string;
  /** Tampilkan label persentase di bawah logo */
  showLabel?: boolean;
  /** Tinggi gelombang (amplitudo) dalam unit viewBox. Default 10. Set 0 untuk fill datar (tanpa gelombang) */
  waveAmplitude?: number;
  /** Lama satu siklus gerak gelombang horizontal dalam detik. Default 2.2 */
  waveSpeedSec?: number;
  className?: string;
}

// Lebar 1 siklus gelombang (unit viewBox). Path digambar berulang beberapa
// siklus + 1 siklus ekstra di kiri-kanan supaya animasi geser horizontal
// looping-nya mulus tanpa ada celah kelihatan di tepi.
const WAVE_CYCLE = 192;
const WAVE_CYCLES = 6; // total lebar path = 6 * 192 = 1152, cukup menutupi viewBox 768 + ruang gerak

function buildWavePath(amplitude: number) {
  if (amplitude <= 0) {
    // fallback: garis datar biasa
    return `M -${WAVE_CYCLE} 0 L ${WAVE_CYCLE * WAVE_CYCLES} 0 L ${WAVE_CYCLE * WAVE_CYCLES} 900 L -${WAVE_CYCLE} 900 Z`;
  }
  const half = WAVE_CYCLE / 2;
  let d = `M -${WAVE_CYCLE} 0 Q -${WAVE_CYCLE - half / 2} -${amplitude} -${half} 0`;
  for (let i = 0; i <= WAVE_CYCLES; i++) {
    const x = -WAVE_CYCLE + half + i * half;
    d += ` T ${x} 0`;
  }
  const rightEdge = WAVE_CYCLE * WAVE_CYCLES;
  d += ` L ${rightEdge} 900 L -${WAVE_CYCLE} 900 Z`;
  return d;
}

/**
 * Logo Pemantik yang "terisi" warna aslinya (kuning + oranye) secara
 * bertahap dari bawah ke atas sesuai progres value/max, dengan animasi
 * halus. Sebelum terisi, logo tampil abu-abu (skeleton state).
 *
 * Contoh pemakaian:
...
export default function PemantikLogoProgress({
  ...
  const viewBoxH = 768;
  const filledHeight = (viewBoxH * displayPercent) / 100;
  const levelY = viewBoxH - filledHeight; // posisi vertikal permukaan air
  const clipId = "pemantik-fill-clip";
  const waveD = buildWavePath(waveAmplitude);
  ...
  return (
    ...
    >
      ...
      >
        <defs>
          <clipPath id={clipId}>
            {/* Group ini yang naik/turun mengikuti progres (transisi dihandle React state di atas) */}
            <g transform={`translate(0, ${levelY})`}>
              <path d={waveD}>
                {waveAmplitude > 0 && (
                  <animateTransform
        ...
        <g transform="translate(249,73)">
          {/* Layer abu-abu (base / belum terisi) */}
          <path fill={emptyColor} d={PATH_YELLOW} />
          <path fill={emptyColor} d={PATH_ORANGE} />

          {/* Layer warna asli, hanya tampak di area yang sudah "terisi" */}
